use anchor_lang::prelude::{AccountInfo, Clock, Pubkey, SolanaSysvar};
use anchor_lang::solana_program::entrypoint::ProgramResult;
use anchor_lang::solana_program::program_pack::Pack;

use self::solend::{LastUpdate, Reserve, ReserveCollateral, ReserveLiquidity, PROGRAM_VERSION};

pub mod solend {
    use std::{cmp::Ordering, convert::{TryInto, TryFrom}};

    use anchor_lang::{
        prelude::{msg, ProgramError},
        solana_program::{
            clock::Slot,
    ...
    impl Reserve {
        ...
        pub fn collateral_exchange_rate(
            ...
            let total_liquidity = self.liquidity.total_supply().unwrap();
        ...
        pub fn accrue_interest(&mut self, current_slot: Slot) -> ScopeResult<()> {
            let slots_elapsed = self.last_update.slots_elapsed(current_slot).unwrap();
            if slots_elapsed > 0 {
                let current_borrow_rate = self.current_borrow_rate().unwrap();
        ...
        pub fn current_borrow_rate(&self) -> ScopeResult<Rate> {
            ...
            if low_utilization || self.config.optimal_utilization_rate == 100 {
                let normalized_rate = utilization_rate.try_div(optimal_utilization_rate).unwrap();
        ...
        fn unpack_from_slice(input: &[u8]) -> std::result::Result<Self, ProgramError> {
            ...
            Ok(Self {
                ...
                last_update: LastUpdate {
                    ...
                    stale: unpack_bool(last_update_stale).unwrap(),
                },
    ...
    impl ReserveLiquidity {
        ...
        fn compound_interest(
            ...
            let slot_interest_rate = current_borrow_rate.try_div(SLOTS_PER_YEAR).unwrap();
            ...
            self.borrowed_amount_wads = self.borrowed_amount_wads.try_add(net_new_debt).unwrap();
            Ok(())
        }

        /// Calculate the liquidity utilization rate of the reserve
        pub fn utilization_rate(&self) -> std::result::Result<Rate, DecimalError> {
            let total_supply = self.total_supply().unwrap();
    ...
    impl ReserveCollateral {
        ...
        fn exchange_rate(
            ...
            let rate = if self.mint_total_supply == 0 || total_liquidity == Decimal::zero() {
                ...
                Rate::try_from(mint_total_supply.try_div(total_liquidity).unwrap()).unwrap()
            };
...
pub fn update(
    ...
    let mut data = ctoken_account.data.borrow_mut();
    let mut reserve = Reserve::unpack(&data).unwrap();

    reserve.last_update.slot = Clock::get().unwrap().slot;
