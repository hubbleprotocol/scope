use crate::{DatedPrice, Price, ScopeResult};
...
pub fn get_price(solend_reserve_account: &AccountInfo, clock: &Clock) -> ScopeResult<DatedPrice> {
    let mut reserve = Reserve::unpack(&solend_reserve_account.data.borrow()).unwrap();
    ...
    let value = scaled_rate(&reserve).unwrap();
...
fn scaled_rate(reserve: &Reserve) -> Result<u64> {
    ...
    let rate = reserve.collateral_exchange_rate().unwrap();
    let value = rate.collateral_to_liquidity(FACTOR).unwrap();
...
#[cfg(test)]
mod test {
    use crate::utils::ctokens::solend::{Reserve, ReserveCollateral, ReserveLiquidity};
