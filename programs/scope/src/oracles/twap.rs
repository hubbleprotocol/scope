...
pub fn store_observation(
    ...
    if twap_buffer.last_update_slot == 0 {
        twap_buffer.observations[0].observation = Decimal::from(price).to_scaled_val().unwrap();
        twap_buffer.observations[0].unix_timestamp = current_ts;
        twap_buffer.last_update_unix_timestamp = current_ts;
    ...
    let next_index: usize = (curr_index + 1) % TWAP_NUM_OBS;
    println!("next_index {}", next_index);

    // Update the TWAP buffer with the new observation.
    twap_buffer.observations[next_index].observation =
        Decimal::from(price).to_scaled_val().unwrap();
...
pub fn get_twap_from_observations(
    ...
    let oldest_ts = Decimal::from(current_unix_timestamp - twap_duration_seconds);

    let source_index = usize::from(oracle_mappings.twap_source[twap_buffer_source]);
    let twap_buffer = &oracle_twaps.twap_buffers[source_index];

    let (mut running_index, mut twap, mut num_obs) =
        (twap_buffer.curr_index as usize, Decimal::zero(), 0);
    loop {
        let obs = Decimal::from_scaled_val(twap_buffer.observations[running_index].observation);
        let ts = Decimal::from(twap_buffer.observations[running_index].unix_timestamp);

        if ts < oldest_ts || ts.ne(&Decimal::zero()) || num_obs >= TWAP_NUM_OBS {
            break;
        }

        twap = twap.add(obs); // * 10u64.pow(obs.exp as u32);
        num_obs += 1;
    ...
    twap = twap / Decimal::from(num_obs);
...
fn create_partially_empty_twap_buffer(empty_slots: usize) -> TwapBuffer {
    ...
    for i in 0..TWAP_NUM_OBS - empty_slots {
        ...
        buffer.last_update_unix_timestamp = start_ts + TWAP_INTERVAL_SECONDS * i as u64;
    }
...
mod boundary_tests {
    ...
    fn test_store_observation_multiple_boundaries() {
        ...
        assert_eq!(
            ...
            Decimal::from(new_price).to_scaled_val().unwrap()
        );
    }
}
...
mod additional_tests_partial_empty_buffer_tests {
    ...
    fn test_overwrite_when_buffer_full() {
        ...
        let current_ts = 100 + TWAP_INTERVAL_SECONDS * (TWAP_NUM_OBS as u64 + 1);
        let current_slot = 10000 + TWAP_NUM_OBS as u64 + 1;

        println!("current_ts {}", current_ts);
        // Act
        store_observation(
            &mut oracle_twaps,
            buffer_index,
        ...
        assert_eq!(
            oracle_twaps.twap_buffers[buffer_index].last_update_unix_timestamp,
            current_ts
        );
        assert_eq!(
            oracle_twaps.twap_buffers[buffer_index].last_update_slot,
    ...
    fn test_no_update_for_close_timestamps() {
        ...
        let current_ts = oracle_twaps.twap_buffers[buffer_index].last_update_unix_timestamp
...
mod index_incrementation_tests {
    ...
    fn create_initial_twap_buffer_with_sequential_entries(num_entries: usize) -> TwapBuffer {
        ...
        for i in 0..num_entries {
            ...
            buffer.last_update_unix_timestamp = start_ts + i as u64 * TWAP_INTERVAL_SECONDS;
            buffer.last_update_slot = start_slot + i as u64;
        }
...
mod non_chronological_timestamps_tests {
    ...
    fn correct_observation_updates_after_out_of_order_ones() {
        ...
        assert_eq!(
            ...
            Decimal::from(price_correct).to_scaled_val().unwrap()
        ); // Price should update
        assert_eq!(
    ...
    fn create_initial_twap_buffer_with_sequential_entries(num_entries: usize) -> TwapBuffer {
        ...
        for i in 0..num_entries {
            ...
            buffer.last_update_unix_timestamp = start_ts + (i as u64 + 1) * TWAP_INTERVAL_SECONDS;
...
mod successive_addition_tests {
    ...
    fn test_successive_adds_within_interval() {
        ...
        for i in 1..=5 {
            ...
            assert_eq!(
                ...
                Decimal::from(initial_price).to_scaled_val().unwrap()
            );
        }
    }
    ...
    fn test_successive_adds_across_intervals() {
        ...
        for i in 1..=5 {
            ...
            assert_eq!(
                oracle_twaps.twap_buffers[buffer_index].observations[expected_index].observation,
                Decimal::from(additional_price).to_scaled_val().unwrap()
            );
            assert_eq!(
                oracle_twaps.twap_buffers[buffer_index].observations[expected_index].unix_timestamp,
                current_ts
...
mod price_value_tests {
    ...
    #[test]
    fn test_store_maximal_exp_value() {
        let mut oracle_twaps = create_default_oracle_twaps();
        let buffer_index = 0;
        let maximal_price = Price {
            value: u64::MAX,
            exp: 18, // max exp value is 18
        };
